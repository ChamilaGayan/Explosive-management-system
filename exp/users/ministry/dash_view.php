...
<tbody>';
$query = $conn->query("SELECT * FROM applicant LEFT JOIN permit ON applicant.id=permit.aplicant_id WHERE ministryApprove='1' ");
if($query->num_rows>0){
...
<tbody>';
$query = $conn->query("SELECT * FROM applicant LEFT JOIN permit ON applicant.id=permit.aplicant_id WHERE gaApprove='0'");
if($query->num_rows>0){
while($row=$query->fetch_assoc()){
  $id=$row["id"];
  $name=$row["fullname"]; 
